...
//
// snprintf("%f") drags in newlib's floating-point formatter, which needs ~400 bytes of stack
// (measured: _svfiprintf_r 152 + _printf_float 88 + _dtoa_r 108 + _Balloc/_malloc_r ~48) and
// is the one call in this path whose cost cannot be read off the -fstack-usage output. A
// display task runs on a kilobyte, so it stays out. Rendering a force reading was the only
// float conversion in the firmware, and it overflowed the stack: the overflow hook disables
// interrupts and spins, which looks exactly like a dead board. It is also the immediate
// neighbour of the SessionController's stack in the heap, whose outermost frame holds the FSM
// -- so an overflow that does not trip the canary lands on the commanded brake duty cycle.
//
